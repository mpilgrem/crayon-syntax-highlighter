...
(function($) {
  CrayonSyntaxThemeEditor = new (function() {
    ...
    var reImportant = /\s+!important$/gim;
    var reSize = /^[0-9-]+px$/;
    var reCopy = /-copy(-\d+)?$/;
        ...
        ok: function(val) {
          CrayonUtil.postAJAX(
            ...
            function(result) {
              var msg =
                result > 0
                  ? strings.submitSucceed
                  : strings.submitFail + " " + strings.checkLog;
              admin.createAlert({
                html: msg
              });
            }
    ...
    base.readCSSInfo = function(cssStr) {
      var infoStr = /^\s*\/\*[\s\S]*?\*\//gim.exec(cssStr);
      var themeInfo = {};
      var match = null;
      var infoRegex = /([^\r\n:]*[^\r\n\s:])\s*:\s*([^\r\n]+)/gim;
    ...
    base.removeExistingCSS = function() {
      ...
      preview
        .find(
          'link[rel="stylesheet"][href*="' + adminSettings.currThemeURL + '"]'
        )
        .remove();
    };
    ...
    base.nameToID = function(name) {
      return name.toLowerCase().replace(/\s+/gim, "-");
    };
    ...
    base.persistAttribute = function(attr, remove_default) {
      remove_default = CrayonUtil.setDefault(remove_default, true);
      base.visitAttribute(attr, function(
        attr,
        elem,
        dataElem,
        dataAttr,
        ...
        if (
          remove_default &&
          attr.prop("tagName") == "SELECT" &&
          attr.val() == attr.attr("data-default")
        ) {
          if (elem) {
    ...
    base.populateAttributes = function($change) {
      ...
      base.getAttributes().each(function() {
        base.visitAttribute($(this), function(
          attr,
          elem,
          dataElem,
          dataAttr,
    ...
    base.updateUI = function() {
      $("#crayon-editor-controls input, #crayon-editor-controls select").bind(
        "change",
        function() {
          changed = true;
        }
