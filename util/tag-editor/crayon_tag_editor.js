...
(function($, wp) {
  ...
  registerBlockType("crayon-syntax-highlighter/crayon", {
    title: "Crayon",
    icon: "editor-code",
    category: "formatting",
    attributes: {
        ...
        if (activeFormat) {
          var findFormat = function(formats) {
            var isFormat = function(el) {
              return el.type == "crayon-syntax-highlighter/code-inline";
            };
  ...
  wp.richText.registerFormatType("crayon-syntax-highlighter/code-inline", {
    title: "Crayon",
    tagName: "span",
  ...
  window.CrayonTagEditor = new (function() {
    ...
    base.init = function() {
      ...
      util = CrayonUtil;
      // This allows us to call $.colorbox and reload without needing a button
      // click.
    ...
    base.bind = function(buttonCls) {
      ...
      $buttons.each(function(i, button) {
        var $button = $(button);
        var $wrapper = $(
          '<a class="crayon-tag-editor-button-wrapper"></a>'
        ).attr("href", s.content_css);
    ...
    base.loadDialog = function(callback) {
      ...
      CrayonUtil.getAJAX(
        { action: "crayon-tag-editor", is_admin: gs.is_admin },
        function(data) {
    ...
    base._showDialog = function(args) {
      ...
      if (base.isCrayon(currNode)) {
        ...
        if (currCrayon.length != 0) {
          ...
          for (var att in atts) {
            ...
            // Update highlights
            setting.change();
            // If global setting changes and we access settings, it should
            // declare loaded settings as changed even if they equal the global
            // value, just so they aren't lost on save
      ...
      ajax_class_timer = setInterval(function() {
        if (
          ...
          !ajax_window.hasClass("crayon-te-ajax")
        ) {
    ...
    base.addCrayon = function() {
      ...
      $(
        "." + gs.changed + "[id],." + gs.changed + "[" + s.data_value + "]"
      ).each(function() {
        var id = $(this).attr("id");
    ...
    base.isCrayon = function(node) {
      return (
        node != null &&
        (node.nodeName == "PRE" ||
          (node.nodeName == "SPAN" && $(node).hasClass(CRAYON_INLINE_CSS)))
      );
    };
