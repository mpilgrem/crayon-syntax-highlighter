...
(function($) {
  window.CrayonSyntaxAdmin = new (function() {
    ...
    base.init = function() {
      ...
      theme_editor_duplicate_button = $(
        "#crayon-theme-editor-duplicate-button"
      );
      ...
      theme_editor_edit_button.click(function() {
        base.show_theme_editor(theme_editor_edit_button, true);
      });
      theme_editor_create_button.click(function() {
        base.show_theme_editor(theme_editor_create_button, false);
      });
      theme_editor_duplicate_button.click(function() {
        CrayonSyntaxThemeEditor.duplicate(
          adminSettings.currTheme,
          adminSettings.currThemeName
        );
      });
      theme_editor_delete_button.click(function() {
        if (!theme_editor_edit_button.attr("disabled")) {
          CrayonSyntaxThemeEditor.del(
            adminSettings.currTheme,
            adminSettings.currThemeName
          );
        }
      ...
      theme_editor_submit_button.click(function() {
        CrayonSyntaxThemeEditor.submit(
          adminSettings.currTheme,
          adminSettings.currThemeName
        );
      });
      ...
      $("#crayon-settings-form").submit(function() {
        ...
        if (last == target) {
          var r = confirm(
            "Please BACKUP your database first! Converting will update your post content. Do you wish to continue?"
          );
          return r;
        }
      });
    ...
    base.refresh_theme_info = function(callback) {
      ...
      adminSettings.currThemeName = theme_select
    ...
    base.get_theme_url = function($id) {
      var url =
        $id in adminSettings.userThemes
    ...
    base.show_theme_info = function(callback) {
      base.refresh_theme_info(function() {
        var info = CrayonSyntaxThemeEditor.readCSSInfo(
          adminSettings.currThemeCSS
        );
        var infoHTML = "";
        for (var id in info) {
          if (id != "name") {
            ...
            if (id != "description") {
              infoHTML +=
                ...
                id +
                ' field">' +
                CrayonSyntaxThemeEditor.getFieldName(id) +
                ":</div>";
            }
            infoHTML +=
              '<div class="' +
    ...
    base.createDialog = function(args, options) {
      ...
      options.open = function() {
        ...
        $(this)
          .parent()
          .find("button:nth-child(" + options.selectedButtonIndex + ")")
          .focus();
