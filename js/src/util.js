...
(function($) {
  CrayonUtil = new (function() {
    ...
    base.initGET = function() {
      ...
      window.currentDir = window.currentURL.substring(
        0,
        window.currentURL.lastIndexOf("/")
      );
    ...
    base.htmlToFirstNode = function(html) {
      return base.htmlToElements(html)[0];
    };
    ...
    /**
     * Returns either black or white to ensure this color is distinguishable
     * with the given RGB hex. This function can be used to create a readable
     * foreground color given a background color, or vice versa. It forms a
     * radius around white where black is returned. Outside this radius, white
     * is returned.
     *
     * @param hex An RGB hex (e.g. "#FFFFFF")
     * @requires jQuery and TinyColor
     * @param args The argument object. Properties:
     *      amount: a value in the range [0,1]. If the distance of the given hex
     *              from white exceeds this value, white is returned. Otherwise,
     *              black is returned.
     *      xMulti: a multiplier to the distance in the x-axis.
     *      yMulti: a multiplier to the distance in the y-axis.
     *      normalizeHue: either falsey or an [x,y] array range. If hex is a
     *                    colour with hue in this range, then normalizeHueXMulti
     *                    and normalizeHueYMulti are applied.
     *      normalizeHueXMulti: a multiplier to the distance in the x-axis if
     *                          hue is normalized.
     *      normalizeHueYMulti: a multiplier to the distance in the y-axis if
     *                          hue is normalized.
    ...
    base.getReadableColor = function(hex, args) {
      args = $.extend(
        {
          ...
          yMulti: 1.5,
          normalizeHue: [20, 180],
          // For colors that appear lighter (yellow, green, light blue) we
          // reduce the distance in the x direction, stretching the radius in
          // the x axis allowing more black than before.
      ...
      if (
        args.normalizeHue &&
        hsv.h > args.normalizeHue[0] &&
        hsv.h < args.normalizeHue[1]
      ) {
