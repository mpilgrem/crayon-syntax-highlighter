...
(function($) {
  ...
  CrayonSyntax = new (function() {
    ...
    base.process = function(c, replace) {
      ...
      crayons[uid].mainHeightAuto =
        crayons[uid].mainStyle.height == "" &&
        crayons[uid].mainStyle["max-height"] == "";
      ...
      var load_func = function() {
        ...
        if (typeof crayons[uid].expanded == "undefined") {
          ...
          if (
            Math.abs(
              crayons[uid].main.outerWidth() - crayons[uid].table.outerWidth()
            ) < 10
          ) {
      ...
      if (c.filter('[data-settings~="no-popup"]').length == 0) {
        crayons[uid].popup_settings = popupWindow(
          popupButton,
          {
      ...
      if (
        toolbar.filter('[data-settings~="mouseover"]').length != 0 &&
        !touchscreen
      ) {
        ...
        // whole code!
        if (
          toolbar.filter('[data-settings~="overlay"]').length != 0 &&
        ...
        // Use .hover() for chrome, but in firefox mouseover/mouseout worked
        // best
      ...
      if (
        !touchscreen &&
        c.filter('[data-settings~="scroll-mouseover"]').length != 0
      ) {
    ...
    var codePopup = function(uid) {
      ...
      settings.data =
        base.getAllCSS() +
        '<body class="crayon-popup-window" style="padding:0; margin:0;"><div class="' +
        clone.attr("class") +
        ' crayon-popup">' +
    ...
    base.removeCssInline = function(string) {
      var reStyle = /style\s*=\s*"([^"]+)"/gim;
      var match = null;
      while ((match = reStyle.exec(string)) != null) {
        var repl = match[1];
        repl = repl.replace(/\b(?:width|height)\s*:[^;]+;/gim, "");
        string = string.sliceReplace(
          match.index,
    ...
    base.getAllCSS = function() {
      ...
      if (css.length == 1) {
        ...
        filtered = css.filter(
          '[href*="crayon-syntax-highlighter"], [href*="min/"]'
        );
      }
    ...
    var crayonSlide = function(
      ...
      if (show == true) {
        ...
        if (
          /*instant == false && */ object.css("margin-top") == "0px" &&
          hideDelay
        ) {
    ...
    base.togglePlain = function(uid, hover, select) {
      ...
      if (
        (main.is(":animated") || plain.is(":animated")) &&
      ...
      visible.stop(true);
      visible.fadeTo(animt(500, uid), 0, function() {
    ...
    base.toggleNums = function(uid, hide, instant) {
      ...
      var h_scroll_visible =
        crayons[uid].table.outerWidth() +
          pxToInt(crayons[uid].table.css("margin-left")) >
        crayons[uid].main.outerWidth();
      var v_scroll_visible =
        crayons[uid].table.outerHeight() > crayons[uid].main.outerHeight();
    ...
    var updateNumsButton = function(uid) {
      if (
        typeof crayons[uid] == "undefined" ||
        typeof crayons[uid].numsVisible == "undefined"
      ) {
        return;
      }
    ...
    var updateWrapButton = function(uid) {
      if (
        typeof crayons[uid] == "undefined" ||
        typeof crayons[uid].wrapped == "undefined"
      ) {
        return;
      }
      if (crayons[uid].wrapped) {
    ...
    var updateExpandButton = function(uid) {
      if (
        typeof crayons[uid] == "undefined" ||
        typeof crayons[uid].expanded == "undefined"
      ) {
        return;
      }
    ...
    var updatePlainButton = function(uid) {
      if (
        typeof crayons[uid] == "undefined" ||
        typeof crayons[uid].plainVisible == "undefined"
      ) {
        return;
      }
    ...
    var initSize = function(uid) {
      if (typeof crayons[uid].initialSize == "undefined") {
        ...
        crayons[uid].innerSize = {
          width: crayons[uid].width(),
          height: crayons[uid].height()
        };
        crayons[uid].outerSize = {
          width: crayons[uid].outerWidth(),
          height: crayons[uid].outerHeight()
        };
        crayons[uid].borderSize = minusSize(
          crayons[uid].outerSize,
          crayons[uid].innerSize
        );
        crayons[uid].initialSize = {
          width: crayons[uid].main.outerWidth(),
          height: crayons[uid].main.outerHeight()
        };
        crayons[uid].initialSize.height += crayons[uid].toolbarHeight;
        crayons[uid].initialOuterSize = addSize(
          crayons[uid].initialSize,
          crayons[uid].borderSize
        );
        crayons[uid].finalSize = {
          width: crayons[uid].table.outerWidth(),
          height: crayons[uid].table.outerHeight()
        };
        crayons[uid].finalSize.height += crayons[uid].toolbarHeight;
        // Ensure we don't shrink
        crayons[uid].finalSize.width = CrayonUtil.setMin(
          crayons[uid].finalSize.width,
          crayons[uid].initialSize.width
        );
        crayons[uid].finalSize.height = CrayonUtil.setMin(
          crayons[uid].finalSize.height,
          crayons[uid].initialSize.height
        );
        crayons[uid].diffSize = minusSize(
          crayons[uid].finalSize,
          crayons[uid].initialSize
        );
        crayons[uid].finalOuterSize = addSize(
          crayons[uid].finalSize,
          crayons[uid].borderSize
        );
    ...
    var toggleExpand = function(uid, expand) {
      ...
      if (expand) {
        if (typeof crayons[uid].expanded == "undefined") {
          initSize(uid);
          crayons[uid].expandTime = CrayonUtil.setRange(
            crayons[uid].diffSize.width / 3,
            300,
            800
          );
        ...
        crayons[uid].animate(
          newSize,
          animt(crayons[uid].expandTime, uid),
          function() {
    ...
    /* Fix weird draw error, causes blank area to appear where scrollbar once
    was. */
